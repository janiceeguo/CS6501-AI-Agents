...
ENDPOINTS:
  POST /register          â€” Agents register themselves here
  GET  /agents            â€” List all registered agents
  GET  /agents?skill=X    â€” Filter agents by skill keyword
  GET  /agents/{name}     â€” Get a specific agent's details
  DELETE /agents/{name}   â€” Remove an agent (instructor use)
  POST /broadcast         â€” Send a task to ALL agents (for trivia, etc.)
  POST /send              â€” Send a task to a specific agent by name
  GET  /health            â€” Registry health check
  POST /reset             â€” Clear all registrations (instructor use)
"""
...
async def register(request: Request):
    ...
    print(f"âœ… Registered: {name} â†’ {url}")
...
async def broadcast(request: Request):
    ...
    print(f"\nðŸ“¢ Broadcasting to {len(online_agents)} agents: {question[:80]}...")
    ...
    for agent in online_agents:
        try:
            ...
            print(f"  âœ… {agent['name']} responded")
            ...
            print(f"  âŒ {agent['name']} failed: {e}")
...
async def send_to_agent(request: Request):
    ...
    print(f"\nðŸ“¨ Sending to {name}: {question[:80]}...")
...
async def dashboard():
    ...
    for a in agents.values():
        ...
        status_icon = "ðŸŸ¢" if a["status"] == "online" else "ðŸ”´"
        rows += f"""
        <tr>
    ...
    while True:
        ...
        for name, agent in list(agents.items()):
            ...
            if agent["health_failures"] >= PRUNE_AFTER_FAILURES:
                ...
                print(f"ðŸ”´ {name} marked offline ({agent['health_failures']} failures)")
...
if __name__ == "__main__":
    ...
    if public_url:
        print(f"\nðŸŒ PUBLIC URL (share this with the class):")
        print(f"   {public_url}")
        print(f"\n   Students: set REGISTRY_URL={public_url}")
        print(f"\nðŸ“‹ Dashboard:    {public_url}")
        print(f"ðŸ“‹ Agent list:   {public_url}/agents")
        print(f"ðŸ“‹ Broadcast:    POST {public_url}/broadcast")
    else:
        print(f"\nâš ï¸  ngrok not detected â€” running on localhost only.")
        print(f"   To expose publicly, run:  ngrok http {PORT}")
        print(f"\nðŸ“‹ Dashboard:    http://localhost:{PORT}")
        print(f"ðŸ“‹ Agent list:   http://localhost:{PORT}/agents")
        print(f"ðŸ“‹ Broadcast:    POST http://localhost:{PORT}/broadcast")

    print(f"\nðŸŸ¢ Ready for agents!\n")
